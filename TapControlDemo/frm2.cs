...
namespace TapControlDemo
{
    public partial class frm2 : Form
    {
        ...
        otomatis();
            ...
            {
                conn.Open();
                cmd = new MySqlCommand("SELECT * FROM route where id like '%" + textBox1.Text + "%' or shiptoname like '%" + textBox1.Text + "%' or address like '%" + textBox1.Text + "%' or area like '%" +textBox1.Text + "%' or address like '%" + textBox1.Text + "%'", conn);
                ds = new DataSet();
                da = new MySqlDataAdapter(cmd);
                da.Fill(ds, "route");
                dataGridView1.DataSource = ds;
